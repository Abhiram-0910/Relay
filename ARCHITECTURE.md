...
- [x] Rate limiting done via KV daily counter in the Worker (not SQLite — no DB under Worker hosting), shared by `POST /api/runs` and (Step 6, F6) `POST /api/models` as two independent buckets (`worker/src/rateLimit.js`). Best-effort atomic: a rare concurrent race from one IP could allow several extra past the cap.
  **Residual (documented, not over-fixed, re-confirmed during the Step 6 security review, F4, 2026-08-11 — the original tradeoff still stands):** independently traced the exact same non-atomic read-then-write race the review flagged — concurrent requests from one IP can all observe the same stale count and all pass the gate, more than "~1 extra" under real concurrency. Deliberately not fixed: this is a cost-abuse guard (CI minutes, the shared `GEMINI_API_KEY`'s free-tier quota, the `GH_PAT`'s dispatch budget), not a security boundary — nothing sensitive is gated behind it. Exact enforcement would need a Durable Object (or Cloudflare's Rate Limiting binding) replacing the KV counter — new infrastructure, not a same-shape patch — and isn't justified without evidence of actual abuse. Revisit if that changes.
